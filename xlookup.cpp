...
void printAll(bool &AA, string &ans_qname, uint16_t &type, uint16_t &dns_class, uint32_t &ttl, uint16_t &rdlen, uint16_t &ANCOUNT)
{
	...
	if (!ns.empty())
	{
		info << "Authoritative name servers: \n\n";
	...
	if (!ipv6s.empty())
	{
		info << "IPv6 addresses" << (cnames.empty() ? "" : " for " + cnames.back()) << ": \n\n";
		for (auto &ip : ipv6s)
		{
			info << ip << '\n';
		}
...
string static parse_ipv6(const uint8_t *packet, size_t &offset)
{
	char buf[INET6_ADDRSTRLEN];
	inet_ntop(AF_INET6, &packet[offset], buf, sizeof(buf));
	offset += 16;
	return string(buf);
}
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < ANCOUNT_resp; i++)
	{
		...
		case (NS):
		{
			ns.push_back(parse_dns_name(response, offset, n));
			break;
		}
		case (AAAA):
		{
			ipv6s.push_back(parse_ipv6(response, offset));
			break;
		}
		default:
